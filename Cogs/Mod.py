...
class Mod(commands.Cog):
    ...
    @commands.command(
        aliases=['purge'],
        help=
        "Deletes a certain number of messages (deletes 10 messages by default)",
        usage="<amount>")
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.has_permissions(manage_messages=True)
    async def clear(self, ctx, amount=10):
        await ctx.channel.purge(limit=amount)
        await ctx.channel.send(f"**{amount}** messages were removed",
                               delete_after=5)
    ...
    async def clear_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            message = (
                f"🚫 This command requires you to either be an Admin or have the `Manage Messages` permission to use it."
            )
    ...
    async def lock_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            message = (
                f"🚫 This command requires you to either be an Admin or have the `Manage Channels` permission to use it."
            )
            await ctx.send(message, delete_after=5)

    @commands.command(aliases=['uk'],
    ...
    async def serverlockdown(self, ctx):
        ...
        for channel in ctx.guild.text_channels:
            await channel.send("⚠️ Server is now in lockdown.",
                               delete_after=60.0)
    ...
    async def serverunlock(self, ctx):
        ...
        for channel in ctx.guild.text_channels:
            await channel.send("⚠️ Server has been unlocked.",
                               delete_after=60.0)
    ...
    async def toggle_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            message = (
                f"🚫 This command requires you to have the `Administrator` permission to use it."
            )
            await ctx.send(message, delete_after=5)
