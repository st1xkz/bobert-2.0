...
class Welcome(commands.Cog):
    ...
    async def welcome_role(self, before, after):
        if before.guild.id == 781422576660250634:
            role = discord.utils.find(lambda r: r.id == 816858066330320897,
                                      after.guild.roles)
    ...
    async def on_member_join(self, member):
        ...
        embed = discord.Embed(
            ...
            description=
            "This is a 13+ voluntary non-professional hangout server based around mental health and mental illness with the purpose to make it easier to connect with new people and friends. To get started, head over to <#785551273734176828>. Once you've found the secret word, type it in <#784967488139558942> to verify that you are human. If you ever want to rejoin, use **[this link](https://discord.gg/nbKzQaK2su)**.",
            color=member.guild.me.color,
            timestamp=datetime.utcnow())
        embed.set_author(name=f"{languages} {member.name}!",
                         icon_url=member.avatar_url)
        embed.set_thumbnail(url=f"{member.guild.icon_url}")
        embed.set_footer(
            text=
            "The highlighted text are hyperlinks and can be clicked/tapped.",
