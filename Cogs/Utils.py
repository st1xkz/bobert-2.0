...
class Utils(commands.Cog):
    ...
    async def remind(self, ctx, time, *, reminder):
        ...
        if seconds == 0:
            ...
            await ctx.send("The minimum duration is 5 minutes.",
                           delete_after=5)
            ...
            embed = discord.Embed(
                title="Reminder Set 🔔",
                description=
                f"Alright {ctx.author.name}, your reminder for \"{reminder}\" has been set and will end in {counter}.",
    ...
    @commands.command(pass_context=True,
                      help="Displays text in code format",
                      usage="<text>")
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    async def quote(self, ctx, msg: discord.Message):
        embed = discord.Embed(title="Message Link",
                              url=f"{msg.jump_url}",
    ...
    async def calculate(self, ctx, expr):
        if "+" in expr:
            ...
            return
        embed = discord.Embed(title="Calculator", color=0xffffff)
    ...
    async def getcolor(self, ctx, *, colour_codes):
        ...
        if len(colour_codes) > 10:
            return await ctx.send(
                "You can only supply and maximum of **10** hex codes.",
                delete_after=5)
        ...
        for colour_code in colour_codes:
            ...
            with io.BytesIO() as file:
                ...
                embed = discord.Embed(title=f"Color {colour_code}",
                                      color=0x2f3136,
                                      timestamp=datetime.utcnow())
    ...
    @commands.command(aliases=['ce'],
                      help="Creates a custom emoji",
                      usage="<media_file> <name>")
    @commands.cooldown(1, 5, commands.BucketType.user)
        ...
        async with self.client.ses.get(url) as r:
            try:
                if r.status in range(200, 299):
                    ...
                    bytes = img.getvalue()
                    emoji = await ctx.guild.create_custom_emoji(image=bytes,
                                                                name=name)
                    await ctx.send(
                        f"Successfully created emoji: <:{emoji.name}:{emoji.id}>"
                    )

                else:
                    await ctx.send(
                        f"Error occurred when making request: {r.status}")
            except discord.HTTPException:
                embed = discord.Embed(
                    title="Something went wrong",
                    description=
                    "Please make sure you have no spaces when naming your emoji, you have the correct file link, and the file size isn't too big. You can upload your emoji up to 128x128 pixels (256kb) but Discord resizes it to 32x32.",
                    color=0xFF4040,
                    timestamp=datetime.utcnow())
                await ctx.send(embed=embed)

    @commands.command(aliases=['de'],
                      help="Deletes specified emoji",
                      usage="<emoji>")
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    async def define(self, ctx, word):
        ...
        if response.status_code == 404:
            ...
            async with ctx.typing():
                embed = discord.Embed(title=f"`{word.lower()}`",
                                      color=ctx.author.color,
                                      timestamp=datetime.utcnow())
                embed.set_thumbnail(
                    url=
    ...
    @commands.command(
        aliases=['cin'],
        help=
        "Creates an invite from a specified channel or the current channel",
