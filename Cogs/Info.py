...
class Info(commands.Cog):
    ...
    @commands.command(aliases=['mc'],
                      help="Displays how many members are in the server")
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.guild_only()
    async def membercount(self, ctx):
        a = ctx.guild.member_count
        embed = discord.Embed(title=f"members in {ctx.guild.name}",
                              description=a,
    ...
    async def serverinfo(self, ctx):
        ...
        embed.add_field(
            ...
            value='All Messages' if ctx.guild.default_notifications is
            discord.NotificationLevel.all_messages else 'Only_mentions')
        embed.add_field(name='Explicit Content Filter',
                        value=str(ctx.guild.explicit_content_filter))
        ...
        embed.set_footer(text=f'Guild ID: {ctx.message.guild.id}',
    ...
    async def userinfo(self, ctx, *, member: discord.Member = None):
        ...
        embed.add_field(
            name="Activity",
            value=
            f"{str(member.activity.type).split('.')[-1].title() if member.activity else 'N/A'} {member.activity.name if member.activity else ''}"
        )
