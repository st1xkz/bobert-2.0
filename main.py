...
async def rules(ctx):
    ...
    embed = discord.Embed(
        title="📌 Drama",
        description=
        ">>> • Do not bring and/or start unnecessary drama in any of the channels. Keep it in DM’s and away from the server, no one wants to start a fight.",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        title="📌 Advertising",
        description=
        ">>> • Advertisements are not allowed. Do **__not__** promote your YouTube channel(s), Discord server(s), Twitch, or any other social media (Partners exempted. For more info regarding Partners, please refer to <#809215806323163136> and check the pins).",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        title="📌 Spam",
        description=
        ">>> • Spam content should be kept in the appropriate channel (<#852067978173481010>).",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        title="📌 Profanity",
        description=
        ">>> • Swearing is allowed as long as it doesn't offend anyone. This is a chill community so try keeping it that way.",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        ...
        description=
        ">>> • Your profile picture and username/nickname must be SFW (Safe for Work). Inappropriate profile pictures or offensive usernames/nicknames that include illegal/NSFW or having slurs/harsh swears in your username/nickname or any special characters that might cause difficulty in pinging you are not allowed. Your name must be typeable on a standard QWERTY keyboard.",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        ...
        description=
        ">>> • Do not scam or dox yourself/other people. Any attempt to scam or dox someone in any way, shape, or form will be severely reprimanded.",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        title="📌 NSFW Content",
        description=
        ">>> • Just like your favorite ocean, let's keep it clean. No NSFW/sexually explicit content, sexually implicit content or language anywhere on this server.",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        ...
        description=
        ">>> • Post in the appropriate channels. Read the channel topics or ask someone if you're confused.",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        ...
        description=
        ">>> • Add content warnings, trigger warnings, or spoil anything that could be potentially harmful or triggering to somebody. *For more info regarding this topic, please refer to <#809215806323163136> and check the pins.*\n\n🆘 If you are being abused, or in danger of harming yourself/others, please refer to the list of hotline/emergency numbers in <#796573173905752074> and seek help immediately as we are not trained professionals.",
        color=0xd9c2ae)
    await ctx.send(embed=embed)
...
@commands.is_owner()
async def ps(ctx):
    embed = discord.Embed(title="↬ 𝐩𝐚𝐬𝐭𝐞𝐥𝐬", )
    await ctx.send(embed=embed)
...
@commands.is_owner()
async def nn(ctx):
    embed = discord.Embed(title="↬ 𝐧𝐞𝐨𝐧𝐬")
    await ctx.send(embed=embed)
...
@commands.is_owner()
async def dk(ctx):
    embed = discord.Embed(title="↬ 𝐝𝐚𝐫𝐤𝐬")
    await ctx.send(embed=embed)
