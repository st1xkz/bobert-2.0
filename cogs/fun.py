...
class fun(commands.Cog):
    ...
    @commands.command(
        aliases=["rn"],
        help="Generates a random number with the specified length of digits",
    )
    ...
    @commands.command(
        description="Puts words into other peoples mouth's", usage="<@member> <text>"
    )
    ...
    @commands.command(
        name="8ball",
        help="Wisdom. Ask a question and the bot will give you an answer",
        usage="<question>",
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    async def meme(self, ctx):
        reddit = praw.Reddit(
            client_id="giMGsy3IEHdBx1eFEcobIg",
    ...
    @commands.command(
        help="Repeats given text for x amount of times", usage="<amount> <text>"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    @commands.command(
        aliases=["mock", "dr"], help="Mocks given text (e.g. HahAhA)", usage="<text>"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def drunkify(self, ctx, *, s):
        ...
        if len(newText) <= 380:
            ...
            try:
                ...
                await ctx.reply(
                    f"Something went wrong! The message may be too large or be malformed."
                )
        ...
        if user_id != None and args != None:
            try:
                ...
                await ctx.channel.send(
                    "Something went wrong! I cannot DM the given user :("
                )
    ...
    @commands.command(
        name="dmall",
        aliases=["dma"],
        help="DMs every user in the server through the bot",
        usage="<text>",
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.guild_only()
        ...
        if args != None:
            ...
            for member in members:
                try:
                    ...
                    await ctx.send(
                        "Something went wrong! I couldn't send the message :("
                    )
    ...
    @commands.command(
        aliases=["ava"],
        help="Displays your/a user's profile picture",
        usage="[@user/user_id]",
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.guild_only()
    async def avatar(self, ctx, *, member: disnake.Member = None):
        ...
        embed = disnake.Embed(
            title=f"{member}'s Avatar", color=0x000100, timestamp=datetime.utcnow()
        )
    ...
    @commands.command(
        aliases=["jumbo"], help="Enlarges a specified emoji", usage="<emoji>"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def enlarge(
        self, ctx, emoji: typing.Union[disnake.Emoji, disnake.PartialEmoji, str]
    ):
    ...
    @commands.command(
        aliases=["fact"],
        help="Random animal facts for dogs, cats, pandas, foxes, birds, and koalas",
        usage="<animal>",
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    @commands.command(
        name="redpanda", aliases=["rpan"], help="Displays an image of a red panda"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def Red_panda(self, ctx):
        ...
        embed = disnake.Embed(
            title="Have a fluffy red panda!", colour=ctx.author.colour
        )
        embed.set_image(url=dogjson["link"])
        await ctx.send(embed=embed)
