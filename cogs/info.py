...
class info(commands.Cog):
    ...
    @commands.command(
        aliases=["mc"], help="Displays how many members are in the server"
    )
    ...
    async def serverinfo(self, ctx):
        ...
        total_chan_and_cat = (
            total_text_channels + total_voice_channels + total_categories
        )
        ...
        embed.add_field(
            name="Creation Date",
            value=ctx.guild.created_at.strftime(f"%a, %#d %B %Y @ %H:%M:%S %p"),
        )
        embed.add_field(
            name="Verification Level", value=str(ctx.guild.verification_level)
        )
        embed.add_field(
            name="Default Message Notifications",
            value="All Messages"
            if ctx.guild.default_notifications is disnake.NotificationLevel.all_messages
            else "Only_mentions",
        )
        embed.add_field(
            name="Explicit Content Filter", value=str(ctx.guild.explicit_content_filter)
        )
        embed.add_field(
            name="Total Boosts", value=f"{ctx.guild.premium_subscription_count}"
        )
        embed.add_field(name="Boost Tier", value=f"{ctx.guild.premium_tier}")
        embed.set_footer(
            text=f"Guild ID: {ctx.message.guild.id}", icon_url=ctx.guild.icon.url
        )
        await ctx.send(embed=embed)
    ...
    @commands.command(
        aliases=["user", "whois"],
        help="Displays given users' info",
        usage="<@user/user_id>",
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.guild_only()
    async def userinfo(self, ctx, *, member: disnake.Member = None):
        ...
        embed.add_field(
            name="Joined", value=member.joined_at.strftime("%a %#d %B %Y, %I:%M %p UTC")
        )
        ...
        embed.add_field(name="Badge", value=f"{hypesquad_class}")
