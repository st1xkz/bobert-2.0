...
class confess(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.channel.id == 806649868314869760:
            ...
            embed = disnake.Embed(
                title="Success",
                description=f"I've received your confession and sent it to the <#806649874379964487> channel!",
                color=0x2F3136,
            )
            ...
            embed = disnake.Embed(
                description=f"**Message deleted in <#806649868314869760>** \n{message.content}",
                color=0xFF4040,
            )
            embed.set_author(
    ...
    @commands.command(
        help="Sends user's confession to the <#806649874379964487> channel through DMs"
    )
        ...
        if ctx.channel.type == disnake.ChannelType.private:
            ...
            try:
                ...
                if msg:
                    ...
                    embed = disnake.Embed(
                        description=f"**Message deleted in DMs** \n{msg.content}",
                        color=0xFF4040,
                    )
                    embed.set_author(
                    ...
                    embed.set_footer(
                        text=f"Author: {ctx.author.id} | Message: {ctx.message.id}"
                    )
                    await log_channel.send(embed=embed)
